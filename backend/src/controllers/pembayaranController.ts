...
export const getAll = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  if (req.user!.role === 'ortu') {
    if (!siswa_id) { res.status(403).json({ success: false, message: 'Akses ditolak: siswa_id wajib untuk role ortu' }); return; }
    res.status(403).json({ success: false, message: 'Akses pembayaran via portal wali murid belum tersedia' }); return;
  }
  if (req.user!.role === 'siswa') {
    if (!siswa_id) { res.status(403).json({ success: false, message: 'Akses ditolak: siswa_id wajib' }); return; }
    const siswa = await Siswa.findByPk(siswa_id as string);
    if (!siswa || siswa.user_id !== req.user!.id) { res.status(403).json({ success: false, message: 'Akses ditolak' }); return; }
  }
...
export const create = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  if (duplikat) { res.status(409).json({ success: false, message: `Tagihan ${jenis_biaya} tahun ${tahun_ajaran} untuk siswa ini sudah ada` }); return; }
  ...
  const pembayaran = await Pembayaran.create({
    siswa_id, tahun_ajaran, jenis_biaya, nominal_biaya, virtual_account,
    va_bank: va_bank || null,
    tanggal_jatuh_tempo: tanggal_jatuh_tempo ? new Date(tanggal_jatuh_tempo) : null,
  });

  res.status(201).json({ success: true, message: 'Tagihan berhasil dibuat', data: pembayaran });
};
...
export const bayar = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  if (reference_number) {
    ...
    if (existing) { res.status(409).json({ success: false, message: 'Reference number sudah diproses' }); return; }
  }
  ...
  const { totalTerbayar, newStatus, pembayaranId, jenisBiaya, tahunAjaran } = await sequelize.transaction(async (t) => {
    ...
    const newStatus: 'belum_bayar' | 'sebagian' | 'lunas' = totalTerbayar >= Number(pembayaran.nominal_biaya) ? 'lunas' : totalTerbayar === 0 ? 'belum_bayar' : 'sebagian';

    await PembayaranDetail.create({ pembayaran_id: pembayaran.id, nominal_bayar, tanggal_bayar: new Date(), bank, reference_number }, { transaction: t });
    await pembayaran.update({ nominal_terbayar: totalTerbayar, status: newStatus, tanggal_bayar: newStatus === 'lunas' ? new Date() : pembayaran.tanggal_bayar, metode_bayar: bank }, { transaction: t });
  ...
  if (newStatus === 'lunas' && process.env.N8N_WEBHOOK_URL) {
    ...
    if (email) {
      fetch(`${process.env.N8N_WEBHOOK_URL}/payment-confirmed`, {
        ...
        body: JSON.stringify({ email, nama_siswa: (siswaData as any)?.siswa?.user?.nama, jenis_pembayaran: jenisBiaya, nominal: totalTerbayar, tahun_ajaran: tahunAjaran }),
...
export const getLaporan = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const list = await Pembayaran.findAll({
    ...
    order: [['tanggal_jatuh_tempo', 'ASC']], limit: 200,
  });

  const { totalTagihan, totalTerbayar } = list.reduce((a, p) => ({
    totalTagihan: a.totalTagihan + Number(p.nominal_biaya),
    totalTerbayar: a.totalTerbayar + Number(p.nominal_terbayar),
  }), { totalTagihan: 0, totalTerbayar: 0 });

  res.json({ success: true, data: list, summary: { total_tagihan: totalTagihan, total_terbayar: totalTerbayar, total_tunggakan: totalTagihan - totalTerbayar } });
};
...
export const webhookBca = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (pembayaran) {
      ...
      await PembayaranDetail.create({ pembayaran_id: pembayaran.id, nominal_bayar: amount, tanggal_bayar: new Date(), bank: 'bca', reference_number: reference });
...
export const webhookMandiri = async (req: AuthRequest, res: Response): Promise<void> => {
  logger.info({ event: 'mandiri_webhook', payload: req.body });
  res.json({ status: 'ok' });
};

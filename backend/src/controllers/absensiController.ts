...
const fetchRekapBase = async (kelas_id: string, b: number, y: number) => {
  ...
  const [siswaList, mapelList, rows] = await Promise.all([
    sequelize.query<any>(
  ...
  return { siswaList, mapelList, byMapel, startDate, endDate, daysInMonth: new Date(y, b, 0).getDate() };
};
...
export const createQrSession = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const existingSession = await QrCodeSession.findOne({ where: { jadwal_pelajaran_id, tanggal, aktif: true } });
  if (existingSession) {
  ...
  const [qr_image, session] = await Promise.all([
    QRCode.toDataURL(qr_data),
    QrCodeSession.create({ id: sessionId, jadwal_pelajaran_id, tanggal: new Date(tanggal), unique_code, qr_data, aktif: true, waktu_mulai: new Date() }),
  ]);
  res.status(201).json({ success: true, data: { ...session.toJSON(), qr_image, jadwal } });
};
...
export const scanQr = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  if (qr_data?.startsWith('GATE:')) {
    ...
    for (const p of (siswaRow.ortu_phones || []) as string[]) {
  ...
  try { parsedQr = JSON.parse(qr_data); } catch {
    res.status(400).json({ success: false, message: 'QR code tidak valid' }); return;
  }

  const session = await QrCodeSession.findByPk(parsedQr.session_id as string);
  if (!session?.aktif) { res.status(400).json({ success: false, message: 'Session QR tidak aktif atau sudah berakhir' }); return; }

  try {
    const absensi = await Absensi.create({
      siswa_id, jadwal_pelajaran_id: parsedQr.jadwal_id, tanggal: new Date(parsedQr.tanggal),
      waktu_hadir: new Date(), status: 'hadir', qr_code_scanned: true,
      latitude: latitude ?? null, longitude: longitude ?? null, created_by: req.user!.id,
    });
    invalidateCache(siswa_id);
    res.status(201).json({ success: true, message: 'Absensi berhasil dicatat', data: absensi });
  } catch (e: any) {
    if (e.name === 'SequelizeUniqueConstraintError') { res.status(400).json({ success: false, message: 'Absensi sudah tercatat' }); return; }
    throw e;
  }
};
...
export const inputCode = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  if (!session) { res.status(400).json({ success: false, message: 'Kode tidak valid atau session sudah berakhir' }); return; }
  ...
  try {
    const absensi = await Absensi.create({
      siswa_id, jadwal_pelajaran_id, tanggal: new Date(tanggal),
      waktu_hadir: new Date(), status: 'hadir', qr_code_scanned: false, input_code: code,
      latitude: latitude ?? null, longitude: longitude ?? null, created_by: req.user!.id,
    });
    invalidateCache(siswa_id);
    res.status(201).json({ success: true, message: 'Absensi berhasil dicatat', data: absensi });
...
export const manualInput = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const absensi = await Absensi.create({ siswa_id, jadwal_pelajaran_id, tanggal: new Date(tanggal), status, catatan, created_by: req.user!.id });
  invalidateCache(siswa_id);
  res.status(201).json({ success: true, message: 'Absensi berhasil dicatat', data: absensi });
};
...
export const update = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  invalidateCache((absensi as any).siswa_id);
...
export const getLaporan = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const summary = absensiList.reduce((acc, a) => { acc[a.status] = (acc[a.status] || 0) + 1; acc.total++; return acc; }, { hadir: 0, sakit: 0, izin: 0, alfa: 0, total: 0 });
...
export const persiapanGuru = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const rows = await sequelize.query<any>(
    `SELECT s.id AS siswa_id, u.nama AS nama_siswa, s.nis,
       COALESCE(ag.keterangan_status, CASE WHEN ag.waktu_masuk IS NOT NULL THEN 'hadir' ELSE 'alfa' END) AS status_gate,
       ag.keterangan AS ket_gate, ab.id AS absensi_id, ab.status AS status_guru, ab.catatan
     FROM siswa s
     ...
     WHERE s.kelas_id = :kelas_id ORDER BY u.nama`,
...
export const bulkGuru = async (req: AuthRequest, res: Response): Promise<void> => {
  const { jadwal_pelajaran_id, tanggal, absensi: list } = req.body as {
    jadwal_pelajaran_id: string; tanggal: string;
  ...
  sequelize.query(
    ...
    { replacements: { uid: req.user!.id, val: JSON.stringify({ jadwal_pelajaran_id, tanggal, jumlah: list.length }) }, type: QueryTypes.INSERT }
  ).catch(err => logger.error({ event: 'activity_log_error', error: err }));

  res.json({ success: true, message: `${list.length} absensi berhasil disimpan` });
};
...
export const izinBulk = async (req: AuthRequest, res: Response): Promise<void> => {
  const { siswa_id, tanggal, catatan, semua_jadwal, jadwal_pelajaran_id } = req.body as {
    siswa_id: string; tanggal: string; catatan?: string; semua_jadwal: boolean; jadwal_pelajaran_id?: string;
  };
  if (!siswa_id || !tanggal || semua_jadwal === undefined) throw createError('siswa_id, tanggal, dan semua_jadwal wajib diisi', 400);
  if (!semua_jadwal && !jadwal_pelajaran_id) throw createError('jadwal_pelajaran_id wajib jika semua_jadwal=false', 400);

  const result = await setIzin({ siswa_id, tanggal, catatan: catatan || null, semua_jadwal, jadwal_pelajaran_id: jadwal_pelajaran_id || null, created_by: req.user!.id });
  res.json({ success: true, message: semua_jadwal ? `Izin dicatat untuk ${result.jadwal_count} jadwal pelajaran` : 'Izin dicatat untuk 1 jadwal pelajaran', data: result });
};

export const bulkKelas = async (req: AuthRequest, res: Response): Promise<void> => {
  const { kelas_id, tanggal, absensi: list } = req.body as {
    kelas_id: string; tanggal: string;
    absensi: Array<{ siswa_id: string; status: string; catatan?: string }>;
  };
  if (!kelas_id || !tanggal || !list?.length) throw createError('Data tidak lengkap', 400);
...
export const rekapWaliKelas = async (req: AuthRequest, res: Response): Promise<void> => {
  const tgl = (req.query.tanggal as string) || new Date().toISOString().split('T')[0];
  ...
  const rows = await sequelize.query<any>(
    `SELECT s.id AS siswa_id, u.nama AS nama_siswa, s.nis,
       COALESCE(ag.keterangan_status, CASE WHEN ag.waktu_masuk IS NOT NULL THEN 'hadir' ELSE 'alfa' END) AS status,
       ag.waktu_masuk, ag.keterangan, ag.lokasi_valid, ag.lokasi_jarak_meter
...
export const downloadRekap = async (req: AuthRequest, res: Response): Promise<void> => {
  const b = parseInt(req.query.bulan as string) || new Date().getMonth() + 1;
  const y = parseInt(req.query.tahun as string) || new Date().getFullYear();
  const kelas_id = req.query.kelas_id as string;
  ...
  const { siswaList, mapelList, byMapel, daysInMonth } = await fetchRekapBase(kelas_id, b, y);
  ...
  const buildSheet = (mapelNama: string, dayMap: Record<string, Record<number, string>>) => {
    ...
    const dataRows: any[][] = [
      [`STUDENT ATTENDANCE LIST - ${namaSekolah} - ${kelas.nama} - ${mapelNama} - ${namaBulan}`],
      headerRow, dayRow,
    ];
    ...
    for (const s of siswaList) {
      ...
      for (let d = 1; d <= daysInMonth; d++) {
        if (new Date(y, b - 1, d).getDay() === 0) { row.push('—'); continue; }
        const st = sdays[d];
        if (!st) row.push('');
    ...
    for (let d = 1; d <= daysInMonth; d++) totalRow.push(new Date(y, b - 1, d).getDay() === 0 ? '—' : (totalPerDay[d] || 0));
  ...
  if (!mapelList.length) {
    XLSX.utils.book_append_sheet(wb, buildSheet('SEMUA', {}), `Rekap ${namaBulan}`);
  } else {
    for (const mp of mapelList) XLSX.utils.book_append_sheet(wb, buildSheet(mp.nama, byMapel[mp.id] || {}), mp.nama.substring(0, 31));
  }

  const buf = XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });
  res.setHeader('Content-Disposition', `attachment; filename="Absensi_${kelas.nama.replace(/\s+/g, '_')}_${namaBulan.replace(/\s+/g, '_')}.xlsx"`);
...
export const getSiswaDetail = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  if (bulan && tahun) {
    where.tanggal = { [Op.between]: [new Date(parseInt(tahun as string), parseInt(bulan as string) - 1, 1), new Date(parseInt(tahun as string), parseInt(bulan as string), 0)] };
  }
...
export const rekapData = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    siswa: siswaList.map(s => {
      ...
      for (const day of days) {
        if (!day.libur) {
          ...
          absensi[day.tgl] = st || '';
          if (st === 'hadir') H++; else if (st === 'sakit') S++; else if (st === 'izin') I++; else if (st === 'alfa') A++;
        }
      }
  ...
  res.json({
    success: true,
    data: {
      kelas: kelas.nama, namaSekolah: (kelas as any).sekolah?.nama || 'Al Fakhir School',
      bulan: b, tahun: y, namaBulan: new Date(y, b - 1, 1).toLocaleString('id-ID', { month: 'long' }),
      days, mapel: mapelData,
    },
  });
};

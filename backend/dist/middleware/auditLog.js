...
exports.logAction = exports.globalAuditLogger = void 0;
...
function resolveAction(method, path) {
    ...
    for (const [m, re, label, table] of map) {
        if (m === method && re.test(p))
            return { action: label, table };
    }
...
function detectAppSource(req) {
    const ua = (req.headers['user-agent'] || '').toLowerCase();
    const header = req.headers['x-app-source'];
    if (header)
        return header;
...
const globalAuditLogger = (req, res, next) => {
